...
const _scoreIsNumeric = check("score", "Score must be a number between 0 and 100").isFloat({ min: 0, max: 100 });
const _feedbackOptional = check("feedback", "Feedback must be a string").optional().isString();

const _scoreOptional = check("score", "Score must be a number between 0 and 100").optional().isFloat({ min: 0, max: 100 });
